...
class StatsWidget:
    def __init__(self):
        ...
        self.index_of_error_label = QLabel(
            f"Error {self.current_error_index + 1}/{results.get_errors_count()}")
        ...
        self.layout.addWidget(self.index_of_error_label,
                              1, 1, 1, 1, Qt.AlignCenter)
    ...
    def _update_img_label(self):
        if results.get_errors_count() > 0:
            ...
            self.index_of_error_label.setText(
                f"Error {self.current_error_index}/{results.get_errors_count()}")
            self.image_label.setText("No errors, darn good classification 💪")
